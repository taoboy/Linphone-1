...
//    identity = [[NSString stringWithFormat:@"sip:%@@%@", username, domain] cStringUsingEncoding:NSUTF8StringEncoding];
    ...
    //    linphone_core_clear_all_auth_info(lc);     // 我也不知道  这为什么会注释掉
    ...
    ms_free(identity);
    NSLog(@"identity   >>>>>> %s",identity);  //显示配置信息
